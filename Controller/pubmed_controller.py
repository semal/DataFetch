...
def scrapy_pmid_abstract(pmid_list):
    ...
    os.system(
        'scrapy crawl fetch_pubmed_abstract -a pmids="%s" '
...
def generate_csv_by_formula(formula):
    pmid_list = search_pubmed(formula)
    os.system('rm output/mentioned_snps.csv')
    conn = connect_db()
    cursor = conn.execute(
        'select pmid, mutations, title, abstract from complete_res '
        'where pmid in (%s)' % ','.join(["'%s'" % pmid for pmid in pmid_list])
    )
    with open('output/mentioned_snps.csv', 'w') as fo:
        csv_writer = csv.writer(fo)
        csv_writer.writerow(['pmid', 'mutations', 'title', 'abstract'])
        for row in cursor:
